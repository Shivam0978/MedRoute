...
export default function AIAssistantPage() {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      const url = "http://localhost:3001/api/symptom-assistant";
      ...
      if (!resp.ok || !resp.body) {
        toast.error("AI service response error. Try again shortly.");
        setBusy(false);
        return;
      }
      ...
      while (!done) {
        ...
        while ((nl = buf.indexOf("\n")) !== -1) {
          ...
          try {
            ...
            const c = p.choices?.[0]?.delta?.content || p.response || p.content;
            if (c) {
              so += c;
              setMessages((m) => {
                const cp = [...m];
                cp[cp.length - 1] = { role: "assistant", content: so };
                return cp;
              });
            }
          } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <div className="mt-6 min-h-[300px] rounded-2xl bg-card border border-border p-4 space-y-4 shadow-soft">
        {messages.length === 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {EXAMPLES.map((ex) => (
                <button
                  key={ex}
                  onClick={() => send(ex)}
                  className="px-3 py-1.5 rounded-full text-xs border border-border bg-muted hover:bg-card hover:border-primary/40 transition"
                >
                  {ex}
                </button>
              ))}
          ...
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm whitespace-pre-wrap ${
                m.role === "user" ? "gradient-primary text-primary-foreground font-medium" : "bg-muted leading-relaxed"
              }`}
            >
              {m.content || "…"}
            </div>
      ...
      <form
        ...
        className="mt-4 flex items-center gap-2 p-2 bg-card border border-border rounded-2xl shadow-soft"
      >
        <input
          value={input}
        ...
        />
        <button type="button" onClick={voice} className="p-2 rounded-lg hover:bg-muted text-muted-foreground" aria-label="Voice">
