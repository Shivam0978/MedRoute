...
export default function BloodBankPage() {
  ...
  const { data: banks = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("http://localhost:3001/api/blood_banks");
      if (!res.ok) return [];
      return res.json();
    },
  });
  ...
  const { data: donors = [] } = useQuery({
    queryKey: ["donors", group, city, user?.id || user?._id],
    enabled: !!user && !!group,
    queryFn: async () => {
      const authToken = token || localStorage.getItem("mr-token");
      const url = `http://localhost:3001/api/blood_donors?blood_group=${encodeURIComponent(group)}&city=${encodeURIComponent(city)}`;
  ...
  const register = async (e) => {
    ...
    try {
      const authToken = token || localStorage.getItem("mr-token");
      const res = await fetch("http://localhost:3001/api/blood_donors", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="bg-card border border-border rounded-2xl p-4 mb-6 flex flex-wrap gap-2 shadow-soft">
        <select value={group} onChange={(e) => setGroup(e.target.value)} className="px-3 py-2 rounded-lg bg-muted text-sm outline-none">
        ...
        <input
          ...
          placeholder="City"
          className="px-3 py-2 rounded-lg bg-muted text-sm flex-1 min-w-[180px] outline-none"
        />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <section>
          ...
          <ul className="space-y-3">
            {filteredBanks.map((b) => (
              <li key={b.id || b._id} className="p-4 rounded-2xl bg-card border border-border">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {(b.available_groups || []).map((g) => (
                        <span key={g} className="text-xs px-2 py-0.5 rounded-full bg-emergency/15 text-emergency font-mono">
                          {g}
                        </span>
                      ))}
                    </div>
          ...
          {!user ? (
            <p className="text-sm text-muted-foreground">
              Please log in to search donor contact details. This protects donors' personal information.
            </p>
          ) : !group ? (
            <p className="text-sm text-muted-foreground">
              Select a blood group above to search for matching donors. Donor lists are not browsable in bulk.
            </p>
          ) : donors.length === 0 ? (
            ...
            <ul className="grid sm:grid-cols-2 gap-3">
              {donors.map((d) => (
                <li key={d.id || d._id} className="p-3 rounded-xl bg-card border border-border">
        ...
        <section>
          ...
          <form onSubmit={register} className="p-5 rounded-2xl bg-card border border-border grid gap-3 shadow-soft">
            <input
              required
              value={donorName}
