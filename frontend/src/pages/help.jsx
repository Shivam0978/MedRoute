    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <section className="mt-10">
        <form onSubmit={submit} className="space-y-4 bg-card border border-border rounded-2xl p-5">
          <h2 className="font-display text-2xl font-bold flex items-center gap-2"><MessageSquare className="size-5 text-primary" />Send a message to admin</h2>
          {user &&
          <p className="text-xs text-muted-foreground">Sending as <span className="font-medium text-foreground">{user.email}</span></p>
          }
          ...
          <Button type="submit" disabled={busy || !user}><Send className="size-4 mr-2" />{busy ? "Sending…" : "Send to admin"}</Button>
      ...
      <p className="text-xs text-muted-foreground mt-6 text-center">For medical emergencies, call <a className="text-emergency font-semibold" href="tel:102">102</a> immediately.</p>
