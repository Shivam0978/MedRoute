...
import { z } from "zod";
import { LifeBuoy, Send, MessageSquare, BookOpen, Stethoscope, MapPin, Bot } from "lucide-react";
...
const FAQ = [
  ...
  { q: "Is my health data secure?", a: "Health records are protected — only you can read or modify your own records." },
];

export default function HelpPage() {
  const { user, token } = useAuth();
  const [busy, setBusy] = useState(false);
  const [form, setForm] = useState({ name: "", category: "general", subject: "", message: "" });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3001/api/contact-messages", {
        method: "POST",
        headers: {
      ...
      toast.success("Message sent to the admin. We'll reply to your account email.");
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Find quick answers or send a message — it goes straight to the admin.</p>
        </div>
      </div>
      ...
      <section className="mt-10">
        <h2 className="font-display text-2xl font-bold flex items-center gap-2">
          <BookOpen className="size-5 text-primary" />FAQ
        </h2>
      ...
      <section className="mt-10">
        <form onSubmit={submit} className="space-y-4 bg-card border border-border rounded-2xl p-5 shadow-soft">
          <h2 className="font-display text-2xl font-bold flex items-center gap-2">
            <MessageSquare className="size-5 text-primary" />Send a message to admin
          </h2>
          {user && (
          ...
          {!user && (
            <p className="text-xs text-muted-foreground">
              You need to <Link to="/login" className="underline text-primary">sign in</Link> to send a message.
            </p>
          )}
        </form>
      </section>
      ...
      <p className="text-xs text-muted-foreground mt-6 text-center">
        For medical emergencies, call <a className="text-emergency font-semibold" href="tel:102">102</a> immediately.
      </p>
