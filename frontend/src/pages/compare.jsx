...
function ComparePage() {
  ...
  { k: "ICU beds available", get: (h) => h.beds?.[0]?.icu_available ?? 0 },
  { k: "Oxygen beds", get: (h) => h.beds?.[0]?.oxygen_available ?? 0 },
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted">
              <tr>
                ...
                {data.map((h) => <th key={h.id} className="p-3 font-semibold">{h.name}</th>)}
              </tr>
            </thead>
            <tbody>
              {rows.map((r) =>
            <tr key={r.k} className="border-t border-border">
                  ...
                  {data.map((h) => <td key={h.id} className="p-3 text-center">{r.get(h)}</td>)}
                </tr>
            )}
            </tbody>
          </table>
